...
class Murphelements {
  ...
  checkBox(value) {
    const checkBoxstuff = cy.get(value).check();
    cy.get(".checktext")
      .should("be.visible")
      .then(() => {
        const asd = Cypress.$(".checktext").text();
        expect(asd).to.match(/^[A-Za-z\s!,]+$/);
      });
...
class Diffselector {
  ...
  selecting(value) {
    const wot = cy.get(".diff").select(value).click;
    return this;
  }
  testReps(value) {
...
class Vodsnpics {
  ...
  vod(value) {
    const vid = cy.get(value).click("center", { force: true });
    cy.wait(3000);
    return this;
  }
  pic(value) {
    const picture = cy.get(value).should("be.visible");
    return this;
  }
}
